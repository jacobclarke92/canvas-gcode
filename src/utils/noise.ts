...
class Grad {
  ...
  public dot2 = (x: number, y: number): number => this.x * x + this.y * y
  public dot3 = (x: number, y: number, z: number): number => this.x * x + this.y * y + this.z * z
}
...
const grad3 = [
  ...
  new Grad(-1, 1, 0),
  new Grad(1, -1, 0),
  new Grad(-1, -1, 0), // prettier-ignore
  new Grad(1, 0, 1),
  new Grad(-1, 0, 1),
  new Grad(1, 0, -1),
  new Grad(-1, 0, -1), // prettier-ignore
  new Grad(0, 1, 1),
  new Grad(0, -1, 1),
  new Grad(0, 1, -1),
  new Grad(0, -1, -1), // prettier-ignore
]
...
export const seedNoise = (seed: number) => {
  if (seed > 0 && seed < 1) {
    // Scale the seed out
    seed *= 65536
  }
