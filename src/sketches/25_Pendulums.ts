...
export default class Pendulums extends Sketch {
  ...
  init() {
    ...
    this.vs.debugLines = new BooleanRange({ disableRandomize: true, initialValue: false })

    this.addVar('closeShapeThresh', { initialValue: 90, min: 20, max: 500, step: 1, disableRandomize: true })
    this.addVar('pendulums', { initialValue: 3, min: 1, max: MAX_PENDULUMS, step: 1, disableRandomize: true })
    this.addVar('pendLengthStart', { initialValue: 30, min: 1, max: 50, step: 1 })
    this.addVar('pendLengthFalloff', { initialValue: 0.5, min: 0.01, max: 1.5, step: 0.01 })

    for (let i = 0; i < MAX_PENDULUMS; i++) {
      this.addVar(`pend${i}speed`, { initialValue: i === 0 ? 3.2 : 1, min: -25, max: 25, step: 0.05 })
      this.angles[i] = 0
    }
  }
  ...
  draw(increment: number): void {
    ...
    for (let i = 0; i < this.vars.speedUp; i++) {
      ...
      if (this.lastPos) {
        ...
        if (this.increment > 100 && pos.distanceTo(this.startPos) < pendLengthStart / closeShapeThresh) {
          this.ctx.beginPath()
