...
class Strand {
  ...
  pts: Point[]
  angle: number
  constructor({ pt, ctx, angle }: { pt: Point; ctx: GCanvas; angle: number }) {
    this.done = false
    this.ctx = ctx
    this.pt = pt
    this.pts = [pt.clone()]
...
export default class FlowAround extends Sketch {
  ...
  initDraw(): void {
    ...
    const { gutterX, gutterY, numLines, numObstacles, obstacleMinSize, obstacleMaxSize } = this.vars
    ...
    for (let i = 0; i < numLines; i++) {
      this.strands[i] = new Strand({
        ...
        angle: randFloatRange(Math.PI, -Math.PI),
      })
    }
  }
